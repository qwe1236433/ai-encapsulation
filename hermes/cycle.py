...
_HERMES_DIR = REPO_ROOT / "hermes"
if str(_HERMES_DIR) not in sys.path:
    sys.path.insert(0, str(_HERMES_DIR))

from crawler_trigger import trigger_crawler  # noqa: E402
from tuner import collect_analytics_snapshot, propose_and_audit_loop  # noqa: E402
...
def load_active_keyword_pool() -> list[str] | None:
    """读最新生效的 keyword_pool；无则返回 None，由 tuner 回退到 DEFAULT_KEYWORD_POOL。"""
    if not KEYWORD_POOL_ACTIVE_JSON.is_file():
        return None
    try:
        obj = json.loads(KEYWORD_POOL_ACTIVE_JSON.read_text(encoding="utf-8"))
        kw = obj.get("keywords") or []
        return list(kw) if isinstance(kw, list) else None
