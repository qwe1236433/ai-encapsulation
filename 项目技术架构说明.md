...
| **`scripts/export_to_xhs_feed.py`** | 原始 JSON(L) → 归一 JSON 数组 | `--dedupe`、`--digest-out`、`--batch-id`；`--validate-mode`；**`--dry-run`**（不写盘）；stderr 打印 `merge_stats` |
...
由 **`train_baseline_v0.py`** 写出，顶层含：`schema` 为 **`feature_schema_v0`**（默认）或 **`feature_schema_v1`**（`--feature-schema v1`）、`feature_names`、`coefficients`、`intercept`、`holdout_roc_auc`、`holdout_brier_score`、`train_test_split`、`input_features_path`、`input_features_sha256`、`features_provenance`、可选 **`labels_spec`**、可选 **`cross_validation`**（`--cv-folds`）。  
v1 在 v0 三特征基础上增加 **`log1p_comment`、`log1p_collect`、`log1p_share`、`age_days`**（由 CSV 派生）。工厂通过 **`XHS_FACTORY_BASELINE_JSON`** 挂载，与 **`linear_clamp_v1`** 按 **`XHS_FACTORY_BASELINE_MODE`** 组合；请求体 **`like_proxy_hint` / `like_proxy`**；v1 还可选 **`comment_proxy_hint`**、**`collect_proxy_hint`**、**`share_proxy_hint`**、**`published_at_hint`**，缺省见 **`XHS_FACTORY_BASELINE_ASSUMED_*`**（`.env.example`）。
...
**差异（渐进默认）**：`merge-xhs-feed.ps1` 在**未**设置 `FLOW_API_EXPORT_VALIDATE_MODE` 时默认 **`--validate-mode report`**。`POST /api/export-feed` 通过 **`load_dotenv`** 读 `.env`：**`.env.example` 已默认 `FLOW_API_EXPORT_VALIDATE_MODE=report`**，新环境复制后 API 与脚本一致；若 `.env` 中无该键则 API 侧为 **`none`**（仅 API，merge 仍为 report）。详见 **`kb/评估与晋升基线.md`**。
